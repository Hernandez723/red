...
export async function tauriFetch(input: RequestInfo | URL, init?: TauriFetchInit): Promise<Response> {
  ...
  try {
    ...
    if (isTauri()) {
      proxyResponse = await invoke<ProxyHttpResponse>("proxy_http_request", {
        input: {
          url,
          method,
          headers,
      ...
      if (!res.ok) {
        throw new Error(`Web proxy returned HTTP ${res.status}`);
      }
      proxyResponse = await res.json() as ProxyHttpResponse;
    }
